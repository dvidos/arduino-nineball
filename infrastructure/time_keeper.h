...
void CTimeKeeper::every_100_milliseconds_interrupt()
{
    ...
    // hoping to abandon this autogenerated events altogether,
    // in preference of direct function callbacks
    if (events_queue_length >= TIME_KEEPER_EVENTS_QUEUE_SIZE - 3)
        return;
    ...
    if (is_half_second) {
        add_to_timeout_events_queue(is_second ? TIMEOUT_EVERY_HALF_SECOND_EVEN : TIMEOUT_EVERY_HALF_SECOND_ODD);
    }

    if (is_second) {
        add_to_timeout_events_queue(count_100_msecs == 20 ? TIMEOUT_EVERY_SECOND_EVEN : TIMEOUT_EVERY_SECOND_ODD);
    }
